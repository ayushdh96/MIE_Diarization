...
After diarization and transcription, the diarized text is sent to **Ozwell AI** by hitting the API endpoint with an API key. Ozwell AI then generates a **summarized version of the conversation**, tailored to the type of interaction chosen by the user:
- **General mode** — produces a concise conversational summary.  
- **Doctor–Patient mode** — uses prompt engineering to produce a medically focused summary where both the doctor’s and patient’s perspectives are highlighted.  

Custom prompt engineering is applied in both cases to reduce hallucinations and ensure the summaries are grounded in the diarized text.

The summarized output is displayed to the user in the **frontend**, which is built with **React + Vite**, styled with **SCSS and Schdcn UI**, and communicates with the backend (Flask + FastAPI) over **CORS**.

## Running the Full Project Locally (Docker)

This repo ships with a **React frontend** and a **Python backend (Flask + FastAPI)**. The two services communicate over **CORS**.  
...
This will spin up two containers on your local system:
    ...
    Backend: Flask + FastAPI (handles diarization, transcription, and summarization requests).
